...
# Set the maximum upload size to 20MB (20 * 1024 * 1024 bytes)
st.set_option('server.maxUploadSize', 20 * 1024 * 1024)

# Upload file and process
uploaded_file = st.file_uploader("Upload a music file", type=["wav", "mp3"])
if uploaded_file is not None:
    # Check if the file size exceeds the limit
    if len(uploaded_file.getvalue()) > st.get_option('server.maxUploadSize'):
        st.error("Error: File size exceeds the limit (20MB).")
    else:
